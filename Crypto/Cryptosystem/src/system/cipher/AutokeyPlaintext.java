...
public class AutokeyPlaintext {
    ...
    /**
     * 根据Autokey Plaintext密码算法对明文进行加密
     *
     * @param plaintext 明文
     * @param key       密钥
     * @return 密文
     ...
     * @return 明文
     */
    public static String decryption(String ciphertext, String inikey) {
        ciphertext = ciphertext.toLowerCase();// 将密文转换成小写
        ciphertext = ciphertext.replaceAll("[^a-z]", "");//去除所有非字母的字符
        String key = dealKey(ciphertext, inikey);
        int len = key.length();
        ...
        for(int j=0;j<=k;j++){//构造真实的key
            for (int i = 0; i < inilen; i++) {
                if(j*inilen+i<len){
                    int row = alpha.indexOf(key.charAt(j*inilen+i));//行号
                    int column = alpha.indexOf(ciphertext.charAt(j*inilen+i));//列号
                    int index;
                    if (row > column) {
                        index = column + 26 - row;
                    } else {
            ...
            for(int i=0;i<inilen;i++){
                if(j*inilen+i<len){
                    inikey+=plain.charAt(j*inilen+i);
                }
                else {
                    break;
                }
            }
            key=dealKey(ciphertext, inikey);
        }
